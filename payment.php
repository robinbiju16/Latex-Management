...
<section class="slide fade-6 kenBurns" data-name="">
 ...
 require("db.php"); 
$mob=$_SESSION['pala_user'];

$p=$_GET['pcod'];
$res=mysqli_query($con,"select * from product where pdtcode='$p'") or die(mysqli_error($con));
	 $or=mysqli_query($con,"select max(ordr_id) from ordr") or die(mysqli_error($con));
...
{
	 echo "<script>alert('product not existing');history.go(-1);</script>";
}?>
...
<label for='id'><b>Order id</b></label>
    <input type='text' value=<?php echo $b; ?> name='id' disabled>
   ...
   <label for='from'><b>Account number of User</b></label>
    ...
    <label for='qty'><b>Enter the quantity</b></label>
    <input type='text' value="1" onChange=""  name="qty" id="qt" required>
    <label for='for'><b>Paymentment for</b></label>
    <input type='text' value=<?php echo $p; ?> name="for" id='pcode' disabled>
     <label for='amnt'><b>Amount you want to pay</b></label>
    <input type='text'name="amnt" value=<?php echo $r['price']?> id="price" disabled>
    

    <button type="submit" name="submit" class="button blue gradient">Submit</button>
    <button type="submit" class="button blue gradient">cancel</button>
  	</form>
   	<?php
 ...
 $i=$_POST['qty']*$r['price'];
		 echo  $_POST['qty']*$r['price'];
$mob=$_SESSION['pala_user'];

$p=$_GET['pcod'];
		 $_SESSION['pcod']=$p;
		   $sql="insert into ordr (mob,qty,status,ordr_id,pdtcode,amnt,dat) values ('$mob','$qty','0','$b','$p','$i','$d')";
$res=mysqli_query($con,$sql) or die(mysqli_error($con));
		 $e=$r['stock']-$qty;
		 $sql1="update product set stock='$e' where pdtcode='$p'";
	$res1=mysqli_query($con,$sql1) or die(mysqli_error($con));	 
		  
	 echo "<script>alert('successfully register');history.go(-2);</script>";
}?>
